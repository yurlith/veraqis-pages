...
self.onmessage = async (ev) => {
  ...
  // Multi-format analysis: read-only, and the only thing sent automatically.
  if (type === REQ.ANALYSE_CONTAINER) {
    if (typeof taskId !== 'string' || !taskId) return;
    ...
    try {
      ...
      const { analyseContainer } = await import('./recovery-engine.js');
      const result = await analyseContainer(file, (p) =>
        send({ type: RES.PROGRESS, taskId, stage: p.stage, done: p.done, total: p.total, message: null })
      );
      send({ type: RES.CONTAINER_ANALYSIS, taskId, result });
    } catch (e) {
      const se = toStudioError(e);
      if (se.code === ERR.CANCELLED) send({ type: RES.CANCELLED, taskId });
      else send({ type: RES.ERROR, taskId, error: se.toJSON() });
  ...
  // Repair: produces bytes, so it is only ever sent by an explicit user action
  // after the plan has been shown. The verdict travels back with the bytes and
  // the main thread has no path to one without the other.
  if (type === REQ.REPAIR_CONTAINER) {
    if (typeof taskId !== 'string' || !taskId) return;
    if (running.has(taskId)) return;
    const controller = new AbortController();
    running.set(taskId, controller);
    try {
      ...
      const { recoverContainer } = await import('./recovery-engine.js');
      const r = await recoverContainer(file, (p) =>
        send({ type: RES.PROGRESS, taskId, stage: p.stage, done: p.done, total: p.total, message: null })
      );
      // The bytes cross as a Blob — a handle, not a copy — and the worker drops
      // its own reference in the same turn it posts.
      const blob = new Blob([r.bytes], { type: 'application/octet-stream' });
      send({
        type: RES.CONTAINER_REPAIR,
        taskId,
        blob,
        outcome: r.outcome,
        payloadEvidence: r.payloadEvidence,
        claim: r.claim,
        report: r.report,
        source: r.source,
      });
